...
// trialEndsAt is optional — pass an ISO string when known, or omit it entirely
// (leave undefined) to avoid overwriting a previously-stored value.
async function setStatus(userId, email, status, trialEndsAt) {
  ...
  const row = {
    user_id: userId || null,
    email: email || null,
    status: status,            // 'trialing' | 'active' | 'canceled' | 'past_due'
    updated_at: new Date().toISOString()
  };
  if (typeof trialEndsAt !== 'undefined') row.trial_ends_at = trialEndsAt;
...
// Fetch a subscription's current status + trial_end straight from Stripe.
// Used right after checkout, since the Checkout Session payload itself doesn't
// carry the resulting subscription's status or trial info.
async function fetchSubscription(subscriptionId) {
  try {
    ...
    if (!r.ok) { console.error('Failed to fetch subscription:', data.error && data.error.message); return null; }
    return data;
  } catch (e) { console.error('fetchSubscription error:', e); return null; }
}
...
function mapStripeStatus(s) {
  ...
  return 'canceled'; // canceled, unpaid, incomplete_expired, etc.
}
  ...
  try {
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        // The Checkout Session itself doesn't carry the subscription's status/trial
        // info, so fetch the real subscription to know if this is a trial or an
        // immediate paid start (e.g. if a promo code skips the trial).
        let status = 'active';
        ...
        await setStatus(userId, email, status, trialEndsAt);
        break;
      }
      case 'customer.subscription.updated': {
        const userId = (obj.metadata && obj.metadata.supabase_user_id) || '';
        const status = mapStripeStatus(obj.status);
        const trialEndsAt = isoOrNull(obj.trial_end);
        await setStatus(userId, '', status, trialEndsAt);
        break;
      }
