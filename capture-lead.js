...
export default async function handler(req, res) {
  // This endpoint is deliberately called from OTHER people's websites (the embeddable
  // widget in widget.js runs on a client's own domain, not ours), so unlike Locale's
  // other internal API routes we can't lock this one to our own origin. No cookies or
  // credentials are used here, so a wide-open Allow-Origin is safe — abuse is instead
  // held back by the per-IP rate limit below plus server-side input validation.
